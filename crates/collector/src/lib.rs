...
use crate::{
    ...
    flow::{
        ...
        enrichment::EnrichmentActorHandle,
        handlers::{FlowOptionsActorHandle, SonataActorHandle},
        ...
        for (endpoint_name, endpoint) in publisher_config.endpoints {
            ...
            match &endpoint {
                ...
                PublisherEndpoint::FlowKafkaAvro(config) => {
                    // TODO: here improve also integrate better config knobs (e.g. enable disable
                    // certain actors?)
                ...
                PublisherEndpoint::FlowKafkaJson(config) => {
                    ...
                    // TODO: at some point add some condition here (when we add the respective
                    // config)
                    let (flow_recv, _) = supervisor_handle
                        .subscribe(publisher_config.buffer_size)
                        .await?;
