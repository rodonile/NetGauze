...
//! serialization-ready structures that match the schema used by the [pmacct](https://github.com/pmacct/pmacct) project.
//!
...
/// `AsSet` segments are wrapped in braces: `{A B C}` (TODO: double-check)
...
fn convert_route_monitoring(
    ...
    let mut as_path_str: Option<String> = None;
    ...
    for attr in update.path_attributes() {
        match attr.value() {
            ...
            PathAttributeValue::AsPath(ap) => {
                as_path_str = Some(fmt_as_path(ap));
            }
            PathAttributeValue::As4Path(ap) => {
                // As4Path supplements AsPath for 2-byte peer backward compat;
                // prefer it only when no AsPath was already set.
                if as_path_str.is_none() {
                    as_path_str = Some(fmt_as4_path(ap));
                }
            }
    ...
    if let Some(mr) = mp_reach {
        ...
        match mr {
            ...
            _ => {
                ...
                // emit a single placeholder without per-prefix detail (TODO: check)
                out.push(build_msg(
                    LogType::Update,
                    afi,
                    safi,
