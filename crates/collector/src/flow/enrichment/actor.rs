...
impl EnrichmentActor {
    ...
    fn enrich(&self, peer_ip: IpAddr, flow: FlowInfo) -> Result<FlowInfo, EnrichmentActorError> {
        ...
        // TODO: remove
        // println!("{}", serde_json::to_string(&enriched_flow).unwrap());
    ...
    fn enrich_ipfix_packet(
        ...
        let mut enriched_sets = Vec::with_capacity(pkt.sets().len());

        for set in pkt.sets() {
            let (id, data_records) = if let ipfix::Set::Data { id, records } = set {
                (id, records)
            } else {
                debug!("Template set received: filter out");
                continue;
            };

            let mut enriched_records = Vec::with_capacity(data_records.len());
            for record in data_records {
                if let Some(enrichment_fields) = self
                    .enrichment_cache
                    .get_enrichment_fields_for_peer(&peer_ip, obs_id, record.fields())
                {
                    enriched_records.push(record.clone().with_fields_added(&enrichment_fields));
                } else {
