...
#[test]
fn test_enrichment_cache_get_enrichment_fields_for_peer() {
    let mut cache = EnrichmentCache::new();
    ...
    let incoming_fields = vec![Field::sourceIPv4Address(Ipv4Addr::new(10, 0, 0, 1))];
    let result = cache.get_enrichment_fields_for_peer(&ip, 1000, &incoming_fields);
...
#[test]
fn test_enrichment_cache_get_enrichment_fields_for_peer_not_found() {
    let mut cache = EnrichmentCache::new();
