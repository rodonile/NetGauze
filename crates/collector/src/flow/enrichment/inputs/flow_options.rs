...
impl FlowOptionsRecordType {
    ...
    fn is_sampling_type(ies: &[IE]) -> bool {
        ...
        // Sampling record if we have size+population OR interval OR sampler/selector
        // related fields
    ...
    fn process_options_record(
        ...
        let clean_record = match self {
            ...
            Self::Vrf => {
                debug!("Received VRF options record from peer {}", peer);
                Self::handle_vrf_record(peer, record)
            }
            // TODO: provide config opition to drop unknown option types (discuss, maybe safer for
            // prod?)
            Self::Unknown(hash) => {
                debug!(
                    "Received unknown options record type with IEs hash {} from peer {}: {:?}",
                    *hash, peer, *record
                );
                record
            }
        ...
        // TODO: think about if we implement a local cache here and we see record
        // changing too much we       also need to issue a delete before...
