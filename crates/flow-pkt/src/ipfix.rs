...
impl Set {
    ...
    // this is not correct for everything, to really determine this we need to know
    // info about the template that was used to decode this record, but for now
    // let's use this ...
    pub fn contains_option_data_records(&self) -> bool {
        match self {
            Self::Data { id: _, records } => !records[0].scope_fields.is_empty(),
            _ => false,
        }
    }
}
