...
use std::{net::Ipv4Addr, borrow::Cow, fs::File};
...
pub fn save_buf_in_pcap(pcap_file_path: &str, buf_vec: &[Vec<u8>]) {
    ...
    for buf in buf_vec {
        // UDP
        let mut udp_buffer = vec![0u8; MutableUdpPacket::minimum_packet_size() + buf.len()].into_boxed_slice();
        ...
        // IPv4
        let mut ipv4_buffer = vec![0u8; MutableIpv4Packet::minimum_packet_size() + udp_buffer.len()].into_boxed_slice();
        let mut ipv4_packet = MutableIpv4Packet::new(&mut ipv4_buffer).unwrap();
        ipv4_packet.set_version(4);
        ipv4_packet.set_header_length(5);
        ipv4_packet.set_total_length((MutableIpv4Packet::minimum_packet_size() + udp_buffer.len()) as u16);
        ...
        // Ethernet
        let mut ethernet_buffer = vec![0u8; MutableEthernetPacket::minimum_packet_size() + ipv4_buffer.len()].into_boxed_slice();
